...
class Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=2, padding=1, down=True):
        ...
        self.block = nn.Sequential(
            ...
            nn.Dropout2d(0.5) if not down else nn.Identity(), # Dropout acts as noise
            activation,
        )
...
class Generator(nn.Module):
    ...
    def __init__(self, in_channels=3+14+14):
        ...
        self.final = Block(64, 3, stride=1, padding=1, kernel_size=3)

    def forward(self, rgb_a, lc_a, lc_b_masked):
        d1 = self.first_layer(torch.cat([rgb_a, lc_a, lc_b_masked], 1))
        d2 = self.down1(d1)
        d3 = self.down2(d2)
        d4 = self.down3(d3)
        d5 = self.down4(d4)
...
def test():
    # g = Generator()
    # # 105038083; old model params 
    # rgb = torch.randn((1, 3, 256, 256))
    # lc = torch.randn((1, 14, 256, 256))
    # print(g(rgb, lc).shape)
    g = Generator(3 + 14 + 14)
    rgb = torch.randn((1, 3, 256, 256))
    lc_a = torch.randn((1, 14, 256, 256))
    lc_b_mask = torch.randn((1, 14, 256, 256))

    print(g(rgb, lc_a, lc_b_mask).shape)
