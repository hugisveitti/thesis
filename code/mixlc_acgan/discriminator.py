...
class Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=2, padding=1, down=True):
        ...
        self.block = nn.Sequential(
            ...
            # noise in the discriminator?
           # nn.Identity() if not down else nn.Dropout2d(0.5), # Dropout acts as noise, see pix2pix
            activation,
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, in_channels=3):
        ...
        self.patch_gan_net = nn.Sequential(
            nn.Conv2d(bottleneck_features, 1, kernel_size=3, padding=1),
            #nn.Sigmoid() # use sigmoid?
        )
    ...
    def forward(self, x):
        ...
        d4 = self.down3(d3)
        #d5 = self.down4(d4)
        patch_gan = self.patch_gan_net(d4)

        x = self.bottleneck2(d4)
