...
class Generator(nn.Module):
    ...
    def __init__(self, concat_at_start=False):
        super(Generator, self).__init__()
        if not concat_at_start:
            self.lc_network = ChannelGenerator(in_channels=14)
            self.rgb_network = ChannelGenerator(3)
        else:
            self.network = ChannelGenerator(in_channels = 3 + 14)

        mult = 1 if concat_at_start else 2
        self.output_layer = nn.Sequential(
            nn.Conv2d(64 * mult , 3, 3, 1,1),
            #nn.ReLU(inplace=True),
            #nn.ConvTranspose2d(64, 3, 4, 2, padding=1),
            nn.Tanh()
        )
        self.concat_at_start = concat_at_start
        

    def forward(self, rgb_a, lc_ab):
        if self.concat_at_start:
            x = torch.cat([rgb_a, lc_ab], 1)
            x = self.network(x)
...
def test_shape():
   ...
   # with torch.cuda.amp.autocast(enabled=True):
    ...
    loss = l1(fake, img_a)
    print(loss)
    print(torch.max(img_a), torch.min(img_a))
    print(torch.max(fake), torch.min(fake))
