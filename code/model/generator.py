...
class Generator(nn.Module):
    ...
    def __init__(self, in_channels = 3 + config.num_classes + 1):
        ...
        self.first_layer = nn.Sequential(
            nn.Conv2d(in_channels, 64, kernel_size=in_channels, stride=1, padding=in_channels//2),
            nn.ELU()
        )
    ...
    def forward(self, lc, binary_mask, rgb):
        x = torch.cat([lc, binary_mask, rgb], dim=1)
...
def test():
    ...
    print("shape of output",g(rgb, lc, lc_mask).shape)
